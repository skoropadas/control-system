  ...
  implements FlBaseControlHost<T[], T | T[]>, FlCompareHost<T>
{
  ...
  private controlChange$: Subject<[FlBaseControl<T | T[]>, T | T[] | null]> =
    new Subject<[FlBaseControl<T | T[]>, T | T[] | null]>();
  ...
  public registerControl(control: FlControl<T | T[]>): void {
  ...
  public unregisterControl(control: FlControl<T | T[]>): void {
    this.controls.delete(control);
  }

  public get controlChange(): Observable<T | T[] | null> {
  ...
  public typedControlChange(
    type: AbstractConstructor<FlBaseControl<T | T[]>>
  ): Observable<T | T[] | null> {
    return this.controlChange$.pipe(
      filter(
        ([control]: [FlBaseControl<T | T[]>, T | T[] | null]) =>
          control instanceof type
      ),
      map(([, value]: [FlBaseControl<T | T[]>, T | T[] | null]) => value)
    );
  }
  ...
  private updateControl(control: FlControl<T | T[]>): void {
    if (control instanceof FlControlSelector) {
      control.writeValueFromHost(
        this.proxyModel.has(control.value) ? control.value : false
      );
    } else {
